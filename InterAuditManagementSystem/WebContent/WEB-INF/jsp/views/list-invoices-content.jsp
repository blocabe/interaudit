...
table.axial td .table.formlist th {
	...
	<table align="center"><tr>
				...
				<td style="border :1px dotted #0066aa;">
							<form name="listForm" action="showInvoicesPage.htm" method="post"  style="margin :2px;">
								...
								<input style="font:10px Verdana, sans-serif;" type="submit" class="button120" value="Search"/>
								 &nbsp;&nbsp;&nbsp;
				                  <!--input style="font:10px Verdana, sans-serif;width:150px;" type="button" class="button120" value="Reminder Invoices"  onclick="sendReminderInvoices()"/-->	
								</form>
				</td></tr></table>
			...
			<br/>
			        ...
			        <table id="ver-zebra" width="100%" cellspacing="0" class="formlist">
									<caption><span style="color:black;">List of invoices ${ fn:length(viewinvoices.invoices) } invoices found...</span></caption>
									<thead>
									<tr>
										<th scope="col"></th>
									...
									<c:set var="totalNoteDeCredit" value="0"/>
									 ...
									 <c:forEach var="item" items="${viewinvoices.invoices}" varStatus="loop">
											...
											<c:choose>
												...
												<c:otherwise>
													...
													<c:set var="totalNoteDeCredit" value="${ totalNoteDeCredit + item.totalNoteDeCredit  }"/>
												</c:otherwise>
											</c:choose>
											...
											<c:choose>
												...
												<c:otherwise>
													...
													<fmt:formatNumber type="currency" currencySymbol="&euro;">
														${item.amountPaidEuro - item.amountEuroTva}
													</fmt:formatNumber>
												</c:otherwise>
											</c:choose>											
											</td>
											...
											<c:choose>
												...
												<c:when test='${item.type eq "SP" }'>												
												 <span style="color:black;">Supplementaire</span>
												</c:when>
												<c:otherwise>
											...
											<c:choose>
												<c:when test='${item.countRappels > 0 }'>												 
												 <span style="color:red;"><strong>${item.countRappels}</strong></span>
												</c:when>												
												<c:otherwise>
												${item.countRappels}
												</c:otherwise>
									...
									<tfoot>
									 ...
									 <tr>
										...
										<%--fmt:formatNumber type="currency" currencySymbol="&euro;">
														${totalPaid - totalFacture + totalNoteDeCredit}
										</fmt:formatNumber--%>
										<fmt:formatNumber type="currency" currencySymbol="&euro;">
  ...
  <div  class="nav_alphabet"  style="text-align:center;font:  10px Verdana, sans-serif;">           
                ...
                <c:if test="${totalPage > 1}">
					...
					<c:set var="url2" value="showInvoicesPage.htm" />
