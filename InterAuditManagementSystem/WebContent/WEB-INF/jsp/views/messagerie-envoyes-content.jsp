...
#navlist li a#current {
		...
		<div class="nav_alphabet" style="background-color: rgb(248, 246, 233); border: 1px solid  #0066aa; text-align:center; padding-top:1pt; font: bold 11px Verdana, sans-serif;">
				<form style="text-align:center; name="form1" action="${ctx}/showMessageEnvoyesPage.htm" method="post" >
					...
					<span style="font:10px Verdana, sans-serif;margin-right:10pt;"><strong>Exercice</strong> 
						<select style="font:10px Verdana, sans-serif;margin-right:10pt;"  name="year">
							<c:forEach var="y" items="${message_envoyes.years}">
							<option value="${y}" <c:if test='${message_envoyes.exercise==y}'> selected</c:if> >${y}</option>
							</c:forEach>
						</select> 
					</span>
					...
					<span style="font:10px Verdana, sans-serif;margin-right:10pt;"><strong>Client</strong> 
						<select style="font:10px Verdana, sans-serif;margin-right:10pt;"  name="mission">
							...
							<c:forEach var="y" items="${message_envoyes.missionOptions}">
							<option value="${y.id}"  <c:if test='${message_envoyes.customerId==y.id}'> selected</c:if> >${y.name}</option>
							</c:forEach>
						</select> 
					</span>
...
<ul id="navlist">
                                <!-- CSS Tabs -->
<li><a  href="${ctx}/showMessageRecusPage.htm"><img src="images/picto_inbox.gif" border="0"/> re�us</a></li>
<li><a id="current" href="${ctx}/showMessageEnvoyesPage.htm"><img src="images/picto_sent.gif" border="0"/> envoy�s</a></li>
</ul>
</div>

<table id="ver-zebra" width="100%" cellspacing="0" class="formlist">
									<caption><span style="color:purple;">Messages envoy�s</span></caption>
									<thead>
									<tr>
									...
									<c:set var="row" value="0"/>
									 <c:forEach var="item" items="${message_envoyes.messages}" varStatus="loop">
										 	<c:choose>
												...
												<t:tooltip>  
														   <t:text><em>
														   ${fn:substring(item.contents,0,10)}...</em></t:text>
														  <t:help width="400" >
												...
												<t:tooltip>  
														   ...
														   <span style="color:purple;cursor:hand;"><i>${fn:substring(item.to,0,20)}...</i></span></a></em></t:text>
														  <t:help >
														   <font color="STEELBLUE"></font> ${item.to}
	...
	<fieldset>    
		<dl>
				...
				<select style="width:300px;" size="1" name="recipient" id="recipient">
                    <c:forEach var="y" items="${message_envoyes.teamMembers}">
	...
	<fieldset>
		<dl>
            ...
            <dd><textarea style="width:450px;" name="description" id="description" rows="16" cols="100"></textarea></dd>
		</dl>
    </fieldset>
