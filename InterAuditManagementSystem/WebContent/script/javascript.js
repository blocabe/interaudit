...
/**
 * Popup code
 * Name changed from popup to POPUP
 * scrollbars=yes added
 */
function openpopup(form, popup, width, height, e) {
	var xx, yy;
	xx = 100;
	yy = 100;
	window.open('about:blank', 'POPUP', 'scrollbars=yes, directories=0, location=0, menubar=0, status=0, toolbar=0, width=' + width + ', height=' + height + ', top=' + yy + ', left=' + xx);
...
function hideDelimiters()
{
	if( document.getElementsByName("fileType")[0].checked)
		document.getElementsByName("fileDelimiter")[0].disabled = false;
	if( document.getElementsByName("fileType")[1].checked)
		document.getElementsByName("fileDelimiter")[0].disabled = true;
}
